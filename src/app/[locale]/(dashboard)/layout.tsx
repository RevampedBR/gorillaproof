...
export default function DashboardLayout({
    ...
    const [sidebarData, setSidebarData] = useState<{ userName: string; userEmail: string; userInitial: string; orgName: string; orgLogoUrl: string | null; brandColor: string; role: string } | null>(null);
    ...
    return (
        <div className="flex h-screen bg-[oklch(0.10_0.03_155)] text-[oklch(0.75_0.04_155)] font-sans selection:bg-emerald-500/30" style={{ '--brand': sidebarData?.brandColor || '#34d399' } as React.CSSProperties}>
            ...
            <aside className="hidden w-[220px] flex-col border-r border-emerald-900/20 bg-[oklch(0.08_0.03_155)] lg:flex z-10 jungle-leaf-texture">
                ...
                <div className="flex h-[52px] items-center gap-2.5 px-4 border-b border-emerald-900/20 shrink-0 relative z-10">
                    <button
                        ...
                        className="flex h-7 w-7 items-center justify-center rounded-lg shadow-lg cursor-pointer transition-transform hover:scale-105"
                        style={{ background: `linear-gradient(135deg, var(--brand), color-mix(in srgb, var(--brand) 60%, black))`, boxShadow: '0 4px 6px -1px color-mix(in srgb, var(--brand) 20%, transparent)' }}
                    >
                        <Image
                            src="/logo-white.png"
                            alt="GP"
                ...
                <div className="px-3 pt-4 pb-2 relative z-10">
                    <Button
                        ...
                        className="w-full h-9 text-white text-[13px] font-medium rounded-lg shadow-sm gap-1.5 transition-all"
                        style={{ backgroundColor: 'var(--brand)', boxShadow: '0 1px 3px color-mix(in srgb, var(--brand) 25%, transparent)' }}
                    >
                ...
                <nav className="flex-1 overflow-y-auto px-3 py-2 space-y-0.5 viewer-styled-scrollbar relative z-10">
                    {navItems.map((item) => (
                        <Link
                            ...
                            style={isActive(item.href) ? { backgroundColor: 'var(--brand-soft)' } : undefined}
                        >
                            <span style={isActive(item.href) ? { color: 'var(--brand)' } : undefined}>{item.icon}</span>
                            {item.label}
                        </Link>
                    ))}
                    ...
                    <div className="pt-1">
                        <Link
                            ...
                            style={isActive("/clients") ? { backgroundColor: 'var(--brand-soft)' } : undefined}
                        >
                            <span style={isActive("/clients") ? { color: 'var(--brand)' } : undefined}>
                                <svg className="h-[18px] w-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                    ...
                    <Link
                        ...
                        style={isActive("/members") ? { backgroundColor: 'var(--brand-soft)' } : undefined}
                    >
                        <span style={isActive("/members") ? { color: 'var(--brand)' } : undefined}>
                            <svg className="h-[18px] w-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" />
                            </svg>
                        </span>
                    ...
                    <Link
                        ...
                        style={isActive("/calendar") ? { backgroundColor: 'var(--brand-soft)' } : undefined}
                    >
                        <span style={isActive("/calendar") ? { color: 'var(--brand)' } : undefined}>
                            <svg className="h-[18px] w-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <button className="flex w-full items-center gap-2.5 rounded-lg px-2 py-2 text-left hover:bg-emerald-500/5 transition-colors">
                                <div className="flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold ring-1" style={{ backgroundColor: 'var(--brand-soft)', color: 'var(--brand)', borderColor: 'var(--brand-glow)' }}>
