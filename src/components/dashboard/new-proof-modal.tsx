...
export function NewProofModal({ open, onOpenChange }: NewProofModalProps) {
    ...
    const reset = () => {
        ...
        setTitle(""); setFiles([]); setError(null);
    };
    ...
    const removeFile = (id: string) => setFiles(p => { const f = p.find(x => x.id === id); if (f?.preview) URL.revokeObjectURL(f.preview); return p.filter(x => x.id !== id); });
    ...
    const handleProofSubmit = (e: React.FormEvent) => {
        ...
        startTransition(async () => {
            try {
                const r = await createProof(selectedProjectId, fd);
